...
rna_genes
#changing rownames
# "Gene_" followed by the row number
rownames(rna_genes) <- paste("Gene", 1:nrow(rna_genes), sep = " ")
rownames(rna_genes)
```
...
```{r}
#creation:
rna_counts_new <- rna_counts

#changin row names and colnames
rownames(rna_clinical) # patient barcodes
nrow(rna_counts) # 60660 rows
ncol(rna_counts) # 473 cols
ncol(rna_clinical) # 172 cols
nrow(rna_clinical) # 473 rows
colnames(rna_counts) # v1, v2

colnames(rna_counts_new) <- rownames(rna_clinical)

# COLUMNS
nrow(rna_genes)
rownames(rna_counts_new) <- rna_genes$gene_id
...
```{r}
# factorize the gender and vital_status columns
colData(rna_se)$gender <- factor(colData(rna_se)$gender)
colData(rna_se)$vital_status <- factor(colData(rna_se)$vital_status)
# applying NA mask
rna_se <- rna_se[, !is.na(colData(rna_se)$gender) & !is.na(colData(rna_se)$vital_status)]
# experimental design
rna_design <- ~ gender + vital_status

# making sure it's not less than 10
low_expr_genes <- rowSums(assay(rna_se)) < 10
rna_se <- rna_se[!low_expr_genes, ]
...
```{r}
dds <- DESeqDataSetFromMatrix(countData = rna_counts_new,
...
top-right genes: High log2 fold changes and low P-values -> upregulated. The farther to the right and the higher up they are, the more significant and greater the fold change.
bottom-right genes: high log2 fold changes but not as low P-values as the top-right genes -> might be upregulated but not statistically significantly.
top-left genes: low P-values and negative high log2 fold changes -> downregulated.
bottom-left genes: negative log2 fold changes but not as low P-values as the top-left genes -> potential downregulation, but it's not statistically significant
