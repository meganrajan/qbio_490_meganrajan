...
```{r setup}
    knitr::opts_knit$set(root.dir = normalizePath("/Users/meganrajan/Documents/qbio490/qbio_490_meganrajan")) 
```
...
```{r}
write.csv(clinic, "/Users/meganrajan/Documents/qbio490/qbio_490_meganrajan/analysis_data/skcm_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/meganrajan/Documents/qbio490/qbio_490_meganrajan/analysis_data/skcm_clinical_data.csv")
```
...
# 1. How many rows are there? How many columns? 470 rows, 66 cols
# 2. What is the data type of $gender? string
# 3. What is the age in years of the 5th patient? 74
# 4. How old was the 2nd patient at their last follow up? HINT: you need to look at data from two columns to answer this 87.08 years
...
lastFollowUp = strtoi(clinic[2,]$last_contact_days_to)
age = strtoi(clinic[2,]$age_at_diagnosis)

yearsSinceFollowUp = lastFollowUp / 365

age + yearsSinceFollowUp #OR IS IT SUBTRACT
...
```{r Exercise 4.1}
# create plot here
plot(clinic$age_at_diagnosis, clinic$breslow_thickness_at_diagnosis, xlab = "Age at Diagnosis", ylab = "Breslow Thickness at Diagnosis", main = "Breslow Thickness based on Age of Diagnosis", col = "blue")
...
```{r Exercise 4.2}
# create plot here
# Remove rows where the age_at_diagnosis column has NA values
clean_age <- clinic[!is.na(clinic$age_at_diagnosis), ]
# strtoi(clean_total$age_at_diagnosis) # convert to integers

# Remove rows where the ethnicity column has NA values using the already cleaned data frame
clean_total <- clean_age[!is.na(clean_age$ethnicity), ]

boxplot(clean_total$age_at_diagnosis ~ clean_total$ethnicity, main = "Age at Diagnosis by Ethnicity", xlab = "Ethnicity", ylab = "Age at Diagnosis")
...
cleaned_clinic$age_at_diagnosis
```
...
cleaned_clinic$death_event <- ifelse(cleaned_clinic$vital_status == "Alive", FALSE, ifelse(cleaned_clinic$vital_status == "Dead", TRUE, NA))
...
# 3. What age group appears to have the greatest rate of survival? Young

# 4. What age group appears to have the worst rate of survival? Old

# 5. What is the approx. 15-year survival rate for "Old" skin cancer patients? 15 years is 5475 days. It's about 0.20.

# 6. What is the p-value? Is it statistically significant and what does that tell us about the relationship between age and skin cancer survival? P value is 0.0024, it is statistically signficiant and shows us that there is a relationship between age and skin cancer survical. The older someone is, the lower their survival probability goes over time.
